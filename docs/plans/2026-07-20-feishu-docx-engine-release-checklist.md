...
- The focused Code live regression passed end to end with Lark CLI 1.0.73 in disposable Agent Drive documents: content/language merge, caption preservation, conflict detection, strict-identity movement, reconcile, deletion, and exact cleanup passed in 114.7 seconds as user and 256.1 seconds as bot. The scenario timeout is 600 seconds so slow shared-runner API responses cannot outlive the test and overlap the next shared-document scenario; each individual CLI command remains capped at 120 seconds.
