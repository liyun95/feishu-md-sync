...
function planTextScopes(input: {
  ...
  const requiresStructuredPlanning = completedCreateRecoveries.length > 0 || treeRecoveries.length > 0 ||
    input.roundTripLosses.some((loss) => loss.state === 'repairable') ||
    [...localEntries, ...remoteEntries, ...desiredEntries].some((entry) => {
      return Array.isArray(entry.block.children) && entry.block.children.some(isFeishuBlock);
    });
  if (input.tracked && input.localBase && input.remoteBase && !requiresStructuredPlanning) {
    const trackedPlan = planTrackedTextScopes({
      parentBlockId: input.parentBlockId,
    ...
    if (!trackedPlan.fallbackReason ||
      (!trackedPlan.fallbackReason.startsWith('tracked text block type changed') &&
        !trackedPlan.fallbackReason.startsWith('tracked block order changed'))) {
      return trackedPlan;
    }
  }
...
function planTrackedTextScopes(input: {
  ...
  for (let index = 0; index < input.desiredEntries.length; index += 1) {
    ...
    operations.push({
      ...
      desiredMarkdown: desired.node.markdown,
      desiredBlocks: [{
        blockType: desired.node.blockType,
        markdown: desired.node.markdown
      }]
    });
  }
...
function analyzeRecordedRoundTripLosses(input: {
  ...
  const hierarchySections = new Set(input.localBase.nodes.flatMap((node) => {
    return node.kind === 'text' && node.children?.length
      ? [JSON.stringify(node.locator.sectionPath)]
      : [];
  }));
  ...
  for (const node of input.localBase.nodes) {
    ...
    // L0 and R0 may intentionally differ at a stable text locator. The tracked
    // planner preserves R0 unless L0 -> L1 changes that scope, so this is not
    // evidence of round-trip loss by itself. Structural hierarchy loss is
    // handled above through explicit correspondence and repair checks.
    if (!hierarchySections.has(JSON.stringify(node.locator.sectionPath))) continue;
  ...
  for (const node of input.localBase.nodes) {
    ...
    const stableAnchors = localStable && remoteStable && hasStableMissingTableAnchors({
    ...
    const message = !localStable
      ...
      : !remoteStable
        ...
        : `recorded local-only table baseline divergence is ambiguous without stable adjacent anchors: ${tableIdentity(node)}`;
    losses.push({
      side: 'local-only',
      nodeKind: 'table',
      locator: node.locator,
    ...
    blockers.push({
      code: localStable && remoteStable ? 'round-trip-loss-ambiguous' : 'round-trip-loss-drift',
      locator: node.locator,
      message
    });
  }
...
function hasStableMissingTableAnchors(input: {
  localBase: SemanticDocument;
  localCurrent: SemanticDocument;
  remoteBase: SemanticDocument;
  remoteCurrent: SemanticDocument;
  ...
  const localCurrentTable = input.localCurrent.nodes.find((node): node is SemanticTable => {
    return node.kind === 'table' && locatorKey(node.locator) === locatorKey(input.baselineTable.locator);
  });
  if (localBaseIndex <= 0 || localBaseIndex >= input.localBase.nodes.length - 1 || !localCurrentTable) return false;
  const localCurrentIndex = input.localCurrent.nodes.indexOf(localCurrentTable);
  if (localCurrentIndex <= 0 || localCurrentIndex >= input.localCurrent.nodes.length - 1) return false;
  ...
  const remoteCurrentPreceding = input.remoteCurrent.nodes.find((node) => {
    return locatorKey(node.locator) === locatorKey(localBasePreceding.locator);
  });
  const remoteCurrentFollowing = input.remoteCurrent.nodes.find((node) => {
    return locatorKey(node.locator) === locatorKey(localBaseFollowing.locator);
  });
  if (!remoteBasePreceding || !remoteBaseFollowing ||
  ...
  const remoteCurrentPrecedingIndex = input.remoteCurrent.nodes.indexOf(remoteCurrentPreceding);
  if (input.remoteBase.nodes[remoteBasePrecedingIndex + 1] !== remoteBaseFollowing ||
    input.remoteCurrent.nodes[remoteCurrentPrecedingIndex + 1] !== remoteCurrentFollowing) return false;
