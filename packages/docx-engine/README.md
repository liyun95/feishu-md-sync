...
The `0.1.x` engine API is the compatibility line for callers declaring `feishu-docx-engine >=0.1.0 <0.2.0`. Published applications may pin one exact patch version for a reproducible execution graph; the unreleased `feishu-md-sync` 0.6 package pins `0.1.0` exactly. Engine and application packages are released independently.
