...
5. Complete one lane at a time with `multisdk export`, profile-guided local or Manta validation, `multisdk verify`, `multisdk diff`, dry-run `multisdk apply`, write `multisdk apply --write -y`, and `multisdk audit`.
6. When reviewed Feishu content should land in a downstream docs repo, run `multisdk land-docs` first as a dry-run, then with `--write`.
7. Repeat for Java, JavaScript, RESTful, and Go, then run `md2feishu multisdk finalize <task-dir>`.
